...
const handleChange = (event) => {
    console.log('dentro de handleChange en Form ')
    setUserData( {  /**el objeto se pisó, por eso retomar un nuevo objeto y copiar lo de antes  para  no perderlo */
      ...userData, /**hago una copia con los 3 puntos  */      
      [event.target.name] : event.target.value
    })
    console.log('despues de la funcion setUserData ')
  setErrors(validation({
  ...userData,
  [event.target.name]: event.target.value
 }))
 console.log('despues de setError, userData -> :', userData, '.')
}
const handleSubmit = (event) => {
/**preventDefault el hacer click en el boton submit, se recarga
* la pagina , esto evita que la pagina no se está recargando
*/
  ...
  <div>
    <form onSubmit = { handleSubmit }>
      <h1>IngresoX</h1>
        ...
        <button className = {style.boton} text='SubmitX' >Submit</button>
    </form>
  </div>
  )
}
